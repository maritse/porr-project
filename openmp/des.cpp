...
uint64_t DES::stringToUint64(const string &str) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        local_result |= static_cast<uint64_t>(static_cast<unsigned char>(str[i])) << ((7 - i) * 8);
...
void DES::keygen(uint64_t key) {
    ...
    //cout << "Time taken by function generating keys: "
...
uint64_t DES::des(uint64_t block, bool decrypt) {
    ...
    // Podział bloku wejściowego na lewą i prawą część po 32 bitów
    uint32_t left = block >> 32;
    uint32_t right = block & 0xFFFFFFFF;
    ...
    for (uint8_t i = 0; i < 16; i++) {
        ...
        uint32_t F = decrypt ? f(local_right, sub_key[15 - i]) : f(local_right, sub_key[i]);

        #pragma omp critical
        {
            feistel(local_left, local_right, F);
        }
    }
...
uint64_t DES::ip(uint64_t block) {
    uint64_t result = 0;
    // omp tutaj bardzo duzo zwieksza
...
uint64_t DES::ip(uint64_t block) {
    ...
    for (uint8_t i = 0; i < 64; i++) {
        ...
        local_result <<= 1;
        local_result |= (block >> (64 - IP[i])) & LB64_MASK;
...
uint64_t DES::fp(uint64_t block) {
    ...
    for (uint8_t i = 0; i < 64; i++) {
        ...
        local_result <<= 1;
        local_result |= (block >> (64 - FP[i])) & LB64_MASK;
...
uint32_t DES::f(uint32_t R, uint64_t k) {
    ...
    for (uint8_t i = 0; i < 48; i++) {
        ...
        uint8_t shift_amount = 32 - EXPANSION[i];
        uint64_t mask = LB32_MASK;
        uint64_t value_part1 = R >> shift_amount;
        uint64_t value_part2 = value_part1 & mask;
...
uint32_t DES::f(uint32_t R, uint64_t k) {
    ...
    /*
    uint32_t f_result = 0;
    #pragma omp parallel for reduction(|:f_result)
    for (uint8_t i = 0; i < 32; i++) {
        uint32_t temp = 0;
        temp <<= 1;
        temp |= (s_output >> (32 - PBOX[i])) & LB32_MASK;
